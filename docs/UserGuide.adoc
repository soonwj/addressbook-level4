...
* The input for `rm/INDEX` is not compulsory. If not given, it will remove the tag from all contacts.
* If `rm/INDEX` is stated, tags from those indexes in the current list will be removed.
* Matching only occurs only for full words e.g. `Family` will not match `Families`.
...
[TIP]
The date must be a valid with the format: year-month-day (yyyy-MM-dd).
