...
**3.** Upon successful authentication on an `import` command, the **HTTP request process** is executed, fetching a list.
of the authenticated user's Google Contacts from Google's servers. +

**4.** The **conversion process** then converts this list to a list of DoC `Person` objects. +

**5.** The list of converted DoC `Person` objects are then added to the `Model` component. +
...
===== Summary
The following diagram summarizes this implementation and illustrates the parallel between the `import` and `export` commands:
