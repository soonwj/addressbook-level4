...
extensions involving external services that require the OAuth2 protocol. An exciting possibility would be an integration with Facebook, for automated
retrieval of extensive contact and event information.
...
The order in this step is exactly reversed for an `export` command, whereby a list of DoC `Person` objects is first retrieved from the `Model` component.
