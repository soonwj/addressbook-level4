...
`UndoableCommand` class. It removes the tag from all the contacts in the current list.

image::EmailSequenceDiagram.png[width="800"]

When the user executes the command removeTag, the RemoveTagCommandParser class will parse the tag and call the `RemoveTagCommand`. In `RemoveTagCommand` the `execute()` function is called.
This will invoke the `deleteTag` function that resides in the `LogicManager`, which will remove the specific tag from all the contacts and updates the storage.

[NOTE]
Note that the `deleteTag` function will raise a `CommandException` if the `tag` is not valid.

==== Design Considerations

**Aspect:** Implementation of `removeTag` +
**Alternative 1 (current choice):** Directly removes the tag from all persons. +
**Pros:** Easier as mulitple person are updated at one go.  +
**Cons:** It removes for everyone. If the tag only has to be removed from one person, the `edit` command has to be used. +
