...
As shown from the diagram above, the UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `PersonListPanel`, `StatusBarFooter`, `BrowserPanel` etc. All these, including the `MainWindow`, inherit from the abstract `UiPart` class.
...
image::EventPersonModelClassDiagram.png[width="800"]
...
DoC currently has two such commands, namely, `import` and `export`, which requires this mechanism. This command structure has also been set-up to support possible future
extensions. +
...
The `import` command uses `Google Contacts` as a source, and retrieves all of the authenticated users' Google Contacts.
A conversion process is then executed with the help of the `GooglePersonConverterUtil`. The newly converted contacts are then stored
to the `Model` component of DoC.

===== Export contacts in DoC to Google Contacts
The `export` command converts all contacts currently stored in DoC, with the help of the `GooglePersonConverterUtil`,
to instances of Googles' `Person` class. The newly converted contacts are then exported to the authenticated users' Google Contacts.

==== Implementation
The implementation of `import` and `export` are very similar. There are 3 sequential steps in the flow of execution, for both commands: +
**1.** The user input of `import` or `export` is parsed by the `AddressBookParser`, and a new instance of an `ImportCommand` or `ExportCommand` is returned to the `LogicManager`,
 which then calls the instances' `execute()` method +
**2.** The execute() method of `import` and `export` then triggers the **authentication process** with the `BrowserPanel` +
**3.** Upon successful authentication on an `import` command, the **HTTP request process** is executed, followed by the  **conversion process**.
The list of converted DoC `Person` objects are then added to the `Model` component.
The order is exactly reversed for an `export` command


 Only the 3 bolded processes will be discussed in further detail as the process for step 1 is the same as all other commands in DoC.

===== Authentication process
The authentication process is the more involved process of the 3. We can understand it better with the help of the following sequence diagram: +

image::AuthenticationSequenceDiagram.png[width="800"]

This is a summary of the steps in the execution flow: +
**1.** The `execute()` method of the `ImportCommand` is called from the `LogicManager`, firing an `Oauth2BrowserRequestEvent` to the `BrowserPanel` +
**2.** The `BrowserPanel` handles this event and navigates to the **authentication URL*** provided by the event +
**3.** The user successfully authenticates and grants DoC read permission of the users' Google Contacts +
**4.** The `BrowserPanel` is redirected to the **success URL*** +
**4.** The `BrowserPanel` detects the URL change to the **success URL*** and extracts the authentication code appended to the URL,
this code is then attached to a `GoogleAuthenticationSuccessEvent` fired to the instance of `ImportCommand` +
**5.** The `ImportCommand` instance handles the `GoogleAuthenticationSuccessEvent` and creates the `GoogleCredential` object +
**6.** The HTTP request process ensues:

===== HTTP request process
With the `GoogleCredential` object, we can now send HTTP requests quite easily using Google's Java API client, specifically
its `PeopleService` class. Its documentation can be found in the following url: +
...
===== Conversion process
The conversion process is handled entirely by the `GooglePersonConverterUtil`. Note-worthy design decisions are listed as follows: +
**Google Person -> DoC Person** +

* Only Google `Person` objects with a non-null name, and a non-null phone number are accepted
* Google `Person` objects without an email or an address are given the constants `INVALID_EMAIL@INVALID.COM`, and
...
* DoC's `Name`, `Phone`, `Email` and `Address` are added as native Google `Person` properties, with similar names. `Tag`
objects are added under the Google `Person` object's `UserDefined` property
* `ProfilePic` cannot be exported due to Google Contact's restrictions
* The produced Google `Person` objects are added to a contact group titled `Imported from DoC`

==== Design Considerations

**Aspect:** How a successful authentication is detected +
**Alternative 1 (Chosen for security):** Redirect to a hosted domain first, then redirect to a Google Contacts page. +
**Pros:** The underlying `HTTP GET` is sent to a domain under DoC's control -> risk of authentication code leak is controlled. +
**Cons:** The BrowserPanel may display an error page temporarily if the hosted domain is down, however this will not be noticeable if redirection occurs fast. +
**Alternative 2:** Redirect to a Google Contacts page immediately. +
**Pros:** No risk of an error page being display. +
...
**Cons:** The UI for GMail is very poor on `JavaFX WebView`.
...
As shown above, when the user executes the command `removeTag`, the `RemoveTagCommandParser` class will parse the tag and indexes if given, then call the `RemoveTagCommand`. In `RemoveTagCommand` the `execute()` function is called.
...
**Alternative 1 (current choice):** Directly removes a certain tag from specified or all contacts. +
**Pros:** Easier as multiple person are updated at one go.  +
**Cons:** It only can remove one tag at a time. If more tags has to be removed from multiple contacts, this function has to be called more times depending on the number of the tags that has to be removed. +
**Alternative 2:** Usage of `edit` command to remove the tags. +
**Pros:** One lesser command for the user to remember. +
**Cons:** It is very tiring for the user to update each and everyone of the contacts in the contact list, if he decided to remove the tag from everyone.
...
As shown above, when the user executes the command `find`, the `FindCommandParser` class will parse the user's input, then call the `FindCommand`. In `FindCommand` the `execute()` function is called.
...
**Alternative 1 (current choice):** Directly find contacts based on both name and tag. +
**Pros:** One function that can be used to search by name and tag.  +
**Cons:** If name and tag are similar, the contacts with both the same name and tag will be shown. +
**Alternative 2:** Having two functions. One for find by name and another for find by tags. +
**Pros:** Easier to find a person by name or by tag, if both name and tag have similar Strings. +
**Cons:** Extra command for the user to remember.
...
The `updateProfilePic`  command works by taking in the index of the chosen `Person` and updating this `Person` according to the `ProfilePic` given. To do so, the `updateProfilePic` command takes in a URL of the image that the profile picture is to be changed to. This will cause the app to instantiate a new `ProfilePic` object and check if the URL is valid and if it is valid, the app will proceed to update the Person with the new profile picture, otherwise an IllegalValueException will be thrown.

In addition, the image will be downloaded into a local directory called: `ProfilePics` that is generated when the app initializes. This directory is created in the same directory as the `src` directory.

The `deleteProfilePic` command works by taking in the index of the chosen `Person` and updating the `ProfilePic` property of the chosen `Person` to the default profile picture.
