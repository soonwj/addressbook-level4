...
**Alternative 1 (Chosen for security):** Use a hosted domain as the redirection URL for a successful authentication.
The hosted domain will be the designated **success URL*** the `BrowserPanel` listens for. After the authentication token is extracted from the URL,
we then redirect to a Google Contacts page. +
**Pros:** The underlying `HTTP GET` is sent to a domain under DoC's control, reducing the risk of authentication token leak. +
...
**Cons:** The underlying `HTTP GET` containing the authentication token is sent to an external domain.

'''

**Aspect:** Duplication handling. +
**Alternative 1 (current choice):** Do not perform any duplication checks in the implementation of the `export` command. +
**Pros:** We avoid the overhead of having to first import all the user's Google contacts, storing them temporarily, then checking them against
DoC's Model component to sieve out non-duplicates. +
**Cons:** Multiple calls of the `export` command will result in duplicates being created on the user's Google contacts. +
**Alternative 2:** Implement a way to track changes within DoC's `Model` component, or a way to record export history.  +
**Pros:** We now can choose to export only modified or new `Person` objects in DoC. +
**Cons:** Cross-checking is still required against a user's Google contacts, as they can be modified externally.
