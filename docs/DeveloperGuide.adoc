...
Commands such as `clear`, `undo` and `redo` can be used to alter events in the list as they deal with code that directly modifies the address book.

An `EventPanel` is incorporated into the address book `MainWindow` to display all the events inside the internal list using `ObservableList<ReadOnlyEvent>`. This process will be explained later on under the section `Event Card`.
